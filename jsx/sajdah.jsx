// Sajdah Ornament Replacement Script
// Replaces Sajdah markers (12-15mm x 21-22mm) with template objects
(function () {
    ...
    function logMessage(msg) {
        try {
            ...
            logFile.open("a");
            logFile.writeln(new Date().toString() + " [Sajdah] " + msg);
            logFile.close();
        } catch(e) {}
    }
    ...
    for (var i = 0; i < warshItems.length; i++) {
        if (warshItems[i].name.toLowerCase() === "sajdah") {
            sajdahItem = warshItems[i];
            break;
        }
    }
    
    if (!sajdahItem) {
        logMessage("No object named 'sajdah' found in template.");
        warshDoc.close(SaveOptions.DONOTSAVECHANGES);
        return;
    }
    
    app.selection = null;
    sajdahItem.selected = true;
    app.copy();
    warshDoc.close(SaveOptions.DONOTSAVECHANGES);
    ...
    var MIN_W = 11 * MM_TO_PT, MAX_W = 16 * MM_TO_PT;
    var MIN_H = 20 * MM_TO_PT, MAX_H = 24 * MM_TO_PT;
    var NAME_SAJDAH = "sajdah";
    ...
    if (total === 0) {
        logMessage(mainDoc.name + " : No Sajdah objects found.");
    ...
    layer.locked = layerWasLocked;
    logMessage("Replaced " + total + " Sajdah objects in " + mainDoc.name);
})();
