// Border Ornament Replacement Script
// Replaces Border markers (102-104mm x 157-159mm) with template objects
(function () {
    ...
    function logMessage(msg) {
        try {
            ...
            logFile.open("a");
            logFile.writeln(new Date().toString() + " [Border] " + msg);
            logFile.close();
        } catch(e) {}
    }
    ...
    for (var i = 0; i < warshItems.length; i++) {
        if (warshItems[i].name.toLowerCase() === "border") {
            borderItem = warshItems[i];
            break;
        }
    }
    
    if (!borderItem) {
        logMessage("No object named 'border' found in template.");
        warshDoc.close(SaveOptions.DONOTSAVECHANGES);
        return;
    }
    
    app.selection = null;
    borderItem.selected = true;
    app.copy();
    warshDoc.close(SaveOptions.DONOTSAVECHANGES);
    ...
    var MIN_W = 102 * MM_TO_PT, MAX_W = 104 * MM_TO_PT;
    var MIN_H = 157 * MM_TO_PT, MAX_H = 159 * MM_TO_PT;
    var NAME_BORDER = "إطار";
    ...
    if (total === 0) {
        logMessage(mainDoc.name + " : No Border objects found.");
    ...
    layer.locked = layerWasLocked;
    logMessage("Replaced " + total + " Border objects in " + mainDoc.name);
})();
