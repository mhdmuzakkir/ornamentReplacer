// Hizb Ornament Replacement Script
// Replaces Hizb markers (12-15mm x 38-40mm) with template objects
(function () {
    ...
    function logMessage(msg) {
        try {
            ...
            logFile.open("a");
            logFile.writeln(new Date().toString() + " [Hizb] " + msg);
            logFile.close();
        } catch(e) {}
    }
    ...
    for (var i = 0; i < warshItems.length; i++) {
        if (warshItems[i].name.toLowerCase() === "hizb") {
            hizbItem = warshItems[i];
            break;
        }
    }
    
    if (!hizbItem) {
        logMessage("No object named 'hizb' found in template.");
        warshDoc.close(SaveOptions.DONOTSAVECHANGES);
        return;
    }
    
    app.selection = null;
    hizbItem.selected = true;
    app.copy();
    warshDoc.close(SaveOptions.DONOTSAVECHANGES);
    ...
    var MIN_H = 38 * MM_TO_PT, MAX_H = 40 * MM_TO_PT;
    var NAME_HIZB = "حزب";
    ...
    if (total === 0) {
        logMessage(mainDoc.name + " : No Hizb objects found.");
    ...
    layer.locked = layerWasLocked;
    logMessage("Replaced " + total + " Hizb objects in " + mainDoc.name);
})();
