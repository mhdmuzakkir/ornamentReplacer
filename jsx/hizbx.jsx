// HizbX Ornament Replacement Script
// Replaces HizbX markers (12-15mm x 41-45mm) with template objects
(function () {
    ...
    function logMessage(msg) {
        try {
            ...
            logFile.open("a");
            logFile.writeln(new Date().toString() + " [HizbX] " + msg);
            logFile.close();
        } catch(e) {}
    }
    ...
    for (var i = 0; i < warshItems.length; i++) {
        if (warshItems[i].name.toLowerCase() === "hizbx") {
            hizbxItem = warshItems[i];
            break;
        }
    }
    
    if (!hizbxItem) {
        logMessage("No object named 'hizbx' found in template.");
        warshDoc.close(SaveOptions.DONOTSAVECHANGES);
        return;
    }
    
    app.selection = null;
    hizbxItem.selected = true;
    app.copy();
    warshDoc.close(SaveOptions.DONOTSAVECHANGES);
    ...
    var MIN_H = 41 * MM_TO_PT, MAX_H = 45 * MM_TO_PT;
    var NAME_HIZBX = "حزبx";
    ...
    if (total === 0) {
        logMessage(mainDoc.name + " : No HizbX objects found.");
    ...
    layer.locked = layerWasLocked;
    logMessage("Replaced " + total + " HizbX objects in " + mainDoc.name);
})();
