...
(function() {
            ...
            newScan.addEventListener('click', function() {
                if (window.DriveScanner && hasNodeJs()) {
                    ...
                    if (result && result.success) {
                        ...
                        updateStatus('No mushafproject folder found on any drive. Please connect Google Drive or browse manually.', 'error');
                    }
                } else {
                    updateStatus('Drive scanner not available', 'error');
                }
            });
        }
    ...
    function init() {
        ...
        var currentVersion = (window.OrnamentUpdater && window.OrnamentUpdater.CURRENT_VERSION) || '3.4.1';
        ...
        // Populate version in About modal (single source of truth)
        var aboutVersion = document.getElementById('aboutVersion');
        if (aboutVersion && window.OrnamentUpdater) {
            aboutVersion.textContent = 'v' + window.OrnamentUpdater.CURRENT_VERSION;
        }
        ...
        // Check if saved folders are accessible
        var filesAccessible = appSettings.mushafFilesFolder && window.DriveScanner && window.DriveScanner.isAccessibleFolder(appSettings.mushafFilesFolder);
        var tasksAccessible = appSettings.mushafTasksFolder && window.DriveScanner && window.DriveScanner.isAccessibleFolder(appSettings.mushafTasksFolder);
        var hasConfiguredFolders = appSettings.mushafFilesFolder || appSettings.mushafTasksFolder;

        if (hasConfiguredFolders && (!filesAccessible || !tasksAccessible)) {
            if (window.DriveScanner && hasNodeJs()) {
                var result = window.DriveScanner.autoDetectAndSave();
                if (result && result.success) {
                    if (result.projectFolder) appSettings.mushafFilesFolder = result.projectFolder;
                    ...
                    if (result.projectFolder && !appSettings.templatesFolder) {
                        ...
                        var p = req('path');
                        var fs2 = req('fs');
                        var tp = p.join(p.dirname(result.projectFolder), 'templates');
                        if (fs2.existsSync(tp)) {
                            appSettings.templatesFolder = tp;
                            MY_TEMPLATES_FOLDER = tp;
                            loadSavedTemplates();
                        }
                    }
                    saveSettings();
                    ...
                    showDriveMissingModal(appSettings.mushafFilesFolder || appSettings.mushafTasksFolder);
                }
            } else {
                showDriveMissingModal(appSettings.mushafFilesFolder || appSettings.mushafTasksFolder);
            }
    ...
    function bindEvents() {
        ...
        if (elements.currentLayersList) {
            elements.currentLayersList.addEventListener('click', function(e) {
                var btn = e.target.closest('.layer-rename-btn');
                if (btn) {
                    var layerName = btn.dataset.layerName;
                    var targetName = btn.dataset.targetName;
                    if (layerName && targetName) {
                        renameLayerToStandard(layerName, targetName);
                    }
                }
            });
        }
    ...
    function renderSourceLayers(layers) {
        ...
        layers.forEach(function(layer, idx) {
            ...
            var badge = '';
            if (isMatched) {
                badge = '<span style="font-size: 10px; color: var(--success);">' + escapeHtml(layer.standard) + '</span>';
            } else if (isPotential) {
                badge = '<span style="font-size: 10px; color: #f1c40f;">Potential ' + escapeHtml(layer.potential) + '</span>';
            }
            var isChecked = isMatched || isPotential ? 'checked' : '';
            var icon = isMatched ? '✓' : (isPotential ? '★' : '?');
    ...
    function renameLayerToStandard(layerName, standardName) {
        if (!layerName || !standardName) return;
        updateStatus('Renaming layer...', 'processing');
        ensureLayerCopyScriptLoaded(function(ok) {
            if (!ok) {
                updateStatus('Layer copy script not loaded', 'error');
                return;
            }
            var script = 'renameLayerByName(' + JSON.stringify(layerName) + ', ' + JSON.stringify(standardName) + ')';
            csInterface.evalScript(script, function(result) {
                try {
                    var data = JSON.parse(result);
                    if (data.success) {
                        scanCurrentLayers();
                        updateStatus('Layer renamed to "' + standardName + '"', 'success');
                    } else {
                        updateStatus(data.error || 'Rename failed', 'error');
                    }
                } catch (e) {
                    updateStatus('Error renaming layer', 'error');
                }
            });
        });
    }
