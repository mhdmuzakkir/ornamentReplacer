// Surah Ornament Replacement Script
// Replaces Surah markers (87-88mm x 9-10mm) with template objects
(function () {
    ...
    function logMessage(msg) {
        try {
            ...
            logFile.open("a");
            logFile.writeln(new Date().toString() + " [Surah] " + msg);
            logFile.close();
        } catch(e) {}
    }
    ...
    for (var i = 0; i < warshItems.length; i++) {
        if (warshItems[i].name.toLowerCase() === "surah") {
            surahItem = warshItems[i];
            break;
        }
    }
    
    if (!surahItem) {
        logMessage("No object named 'surah' found in template.");
        warshDoc.close(SaveOptions.DONOTSAVECHANGES);
        return;
    }
    
    app.selection = null;
    surahItem.selected = true;
    app.copy();
    warshDoc.close(SaveOptions.DONOTSAVECHANGES);
    ...
    var MIN_W = 87 * MM_TO_PT, MAX_W = 88 * MM_TO_PT;
    var MIN_H = 9 * MM_TO_PT, MAX_H = 10 * MM_TO_PT;
    var NAME_SURAH = "سورة";
    ...
    if (total === 0) {
        logMessage(mainDoc.name + " : No Surah objects found.");
    ...
    layer.locked = layerWasLocked;
    logMessage("Replaced " + total + " Surah objects in " + mainDoc.name);
})();
