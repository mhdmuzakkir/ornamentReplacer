...
(function() {
    ...
    var batchStopped = false;          // graceful stop flag (saves state)
    var BATCH_STATE_FILE = 'batch-state.json';

    // === BATCH STATE PERSISTENCE ===
    function getBatchStatePath() {
        try {
            var extPath = csInterface.getSystemPath('extension').replace(/\\/g, '/');
            return extPath + '/' + BATCH_STATE_FILE;
        } catch (e) {
            return '';
        }
    }
    ...
    function loadBatchState() {
        var statePath = getBatchStatePath();
        if (!statePath) return null;
        try {
            var fs = require('fs');
            if (fs.existsSync(statePath)) {
                var data = fs.readFileSync(statePath, 'utf-8');
    ...
    function hasPausedBatch() {
        var state = loadBatchState();
    ...
    var elements = {
        ...
        useTemplateOnceBtn: document.getElementById('useTemplateOnceBtn'),
        ...
        deleteOldSwatches: document.getElementById('deleteOldSwatches'),
        ...
        fixCurrentLayerOrderBtnLayer: document.getElementById('fixCurrentLayerOrderBtnLayer'),
        fixOrderRiwayahSelectLayer: document.getElementById('fixOrderRiwayahSelectLayer'),
        fixRiwayahLayerOrderBtnLayer: document.getElementById('fixRiwayahLayerOrderBtnLayer'),
    ...
    var CHANGELOG = {
        '3.4.1': {
            ...
            new: [],
            fixed: [
                '"Don\'t Save" mode now correctly skips saving in single-file, batch, and layer-order-fix operations',
                'Layer Order Fix for riwayah respects the selected save mode (no longer overwrites when set to Don\'t Save)'
            ],
            improved: []
        },
        '3.4.0': {
    ...
    function init() {
        ...
        toggleFixLayerOrderSection();
        updateExecutionOrderBadge();
        updateStatus('Ready');
        updateOrnamentsHeader();

        // Check for paused batch
        var pausedState = loadBatchState();
        if (pausedState && pausedState.status === 'paused') {
            showResumeBanner(pausedState);
        }
    ...
    function bindEvents() {
        if (elements.useTemplateOnceBtn) {
            elements.useTemplateOnceBtn.addEventListener('click', useTemplateOnce);
        }
        ...
        var fixLayerOrderAction = document.getElementById('fixLayerOrderAction');
        if (ayahAlignAction) {
            ayahAlignAction.addEventListener('change', function() {
                saveActionSettings();
                updateExecutionOrderBadge();
            });
        }
        ...
        if (fixLayerOrderAction) {
            fixLayerOrderAction.addEventListener('change', function() {
                toggleFixLayerOrderSection();
                saveActionSettings();
                updateExecutionOrderBadge();
            });
        ...
        // Progress overlay buttons
        var progressStopBtn = document.getElementById('progressStopBtn');
        var progressCancelBtn = document.getElementById('progressCancelBtn');
        if (progressStopBtn) {
            progressStopBtn.addEventListener('click', stopBatch);
        }
        if (progressCancelBtn) {
            progressCancelBtn.addEventListener('click', cancelBatch);
        }

        // Batch preview confirm
        var confirmBatchBtn = document.getElementById('confirmBatchBtn');
        if (confirmBatchBtn) {
            confirmBatchBtn.addEventListener('click', function() {
                hideBatchPreviewModal();
                startBatchFromPreview();
            });
        }

        // Resume banner
        var resumeBatchBtn = document.getElementById('resumeBatchBtn');
        var dismissResumeBtn = document.getElementById('dismissResumeBtn');
        if (resumeBatchBtn) {
            resumeBatchBtn.addEventListener('click', function() {
                var state = loadBatchState();
                if (state) resumeBatch(state);
            });
        }
        if (dismissResumeBtn) {
        ...
        if (elements.fixCurrentLayerOrderBtnLayer) {
            elements.fixCurrentLayerOrderBtnLayer.addEventListener('click', fixCurrentLayerOrder);
        }
        if (elements.fixRiwayahLayerOrderBtnLayer) {
            elements.fixRiwayahLayerOrderBtnLayer.addEventListener('click', fixRiwayahLayerOrderLayer);
        }
    ...
    function toggleFixLayerOrderSection() {
        var section = document.getElementById('fixLayerOrderSection');
        var fixAction = document.getElementById('fixLayerOrderAction');
        if (section && fixAction) {
            section.style.display = fixAction.checked ? 'block' : 'none';
        }
    }

    function updateExecutionOrderBadge() {
        var badge = document.getElementById('executionOrderBadge');
        var itemsContainer = document.getElementById('executionOrderItems');
        if (!badge || !itemsContainer) return;

        var actions = [];
        var ayah = document.getElementById('ayahAlignAction');
        var ornament = document.getElementById('ornamentReplacementAction');
        var fixLayer = document.getElementById('fixLayerOrderAction');

        if (ayah && ayah.checked) actions.push('Ayah Align');
        if (ornament && ornament.checked) actions.push('Ornament Replacement');
        if (fixLayer && fixLayer.checked) actions.push('Fix Layer Order');
        ...
        actions.forEach(function(name, i) {
            if (i > 0) {
                html += '<span class="order-arrow">→</span>';
            }
            html += '<span class="order-item">' + (i + 1) + '. ' + name + '</span>';
        });
        itemsContainer.innerHTML = html;
    }

    // === BATCH PREVIEW MODAL ===
    function showBatchPreviewModal(files, options) {
        var modal = document.getElementById('batchPreviewModal');
        if (!modal) return;

        var chunkSize = 6;
        var chunkCount = Math.ceil(files.length / chunkSize);
        var selectedRiwayah = document.getElementById('riwayahSelect') ? document.getElementById('riwayahSelect').value : '';
        var juzCount = document.getElementById('juzFilterCount') ? document.getElementById('juzFilterCount').textContent : '';
        var surahCount = document.getElementById('surahFilterCount') ? document.getElementById('surahFilterCount').textContent : '';

        document.getElementById('previewFileCount').textContent = files.length;
        document.getElementById('previewChunkCount').textContent = chunkCount;
        document.getElementById('previewRiwayah').textContent = selectedRiwayah || 'All';

        var filters = [];
        if (juzCount) filters.push('Juz: ' + juzCount);
        if (surahCount) filters.push('Surah: ' + surahCount);
        document.getElementById('previewFilters').textContent = filters.length ? filters.join(', ') : 'None';

        var actions = [];
        if (options.runAyahAlign) actions.push('Ayah Align');
        if (options.runOrnamentReplacement) actions.push('Ornament Replacement');
        if (options.runFixLayerOrder) actions.push('Fix Layer Order');
        document.getElementById('previewActions').textContent = actions.join(', ') || 'None';
    ...
    function updateProgressOverlay(chunkIdx, totalChunks, currentFile, processed, errors, totalFiles, selectedTypes) {
        ...
        document.getElementById('progressChunk').textContent = chunkIdx + ' / ' + totalChunks;
        document.getElementById('progressFiles').textContent = processed + ' / ' + totalFiles;
        document.getElementById('progressErrors').textContent = errors;
        document.getElementById('progressCurrentFile').textContent = currentFile || 'Processing...';
        var typesEl = document.getElementById('progressSelectedTypes');
        if (typesEl && selectedTypes && selectedTypes.length > 0) {
            typesEl.textContent = 'Types: ' + selectedTypes.join(', ');
        } else if (typesEl) {
            typesEl.textContent = '';
        }
    }
    ...
    function setProgressStatus(msg) {
        var el = document.getElementById('progressStatusMessage');
    ...
    function showResumeBanner(state) {
        ...
        var progress = state.currentChunkIndex * 6;
        var total = state.totalFiles ? state.totalFiles.length : 0;
        text.textContent = 'Paused batch: ' + progress + '/' + total + ' files processed';
            ...
            if (result && result !== 'null' && result !== 'undefined' && result !== '') {
                try {
                    ...
                    if (response.success && response.path) {
                        ...
                        refreshDesigns();
                        updateStatus('Template loaded (once): ' + response.name, 'success');
                    } else {
    ...
    function renderSavedTemplates() {
        ...
        allTemplates.forEach(function(item) {
            ...
            html += '<label class="template-radio-label ' + (isActive ? 'active' : '') + '">' +
                    ...
                    '<span class="template-radio-box">' +
                    '<span class="template-radio-text" title="' + displayName + '">' + displayName + '</span>';
    ...
    var pendingBatch = null; // stores {options, files} between preview and start

    function scanBatchFiles(options) {
        if (!hasNodeJs()) {
    ...
    function processBatchFolderByFolder(options, filesToProcess, resumeState) {
        ...
        var current = resumeState ? resumeState.currentChunkIndex : 0;
        var totalProcessed = resumeState ? (resumeState.completedFiles ? resumeState.completedFiles.length : 0) : 0;
        var totalErrors = resumeState ? (resumeState.failedFiles ? resumeState.failedFiles.length : 0) : 0;
        var allDetails = resumeState ? (resumeState.details || []) : [];
        var completedFiles = resumeState ? (resumeState.completedFiles || []) : [];
        var failedFiles = resumeState ? (resumeState.failedFiles || []) : [];
        ...
        // Save initial state
        var state = {
            batchId: resumeState ? resumeState.batchId : ('batch-' + Date.now()),
            status: 'running',
            ...
            folderPath: options.batchFolder
        };
        saveBatchState(state);

        function finishBatch(finalStatus) {
        ...
        function processNext() {
            if (batchCancelled) {
                ...
                state.currentChunkIndex = current;
                state.completedFiles = completedFiles;
                state.failedFiles = failedFiles;
                state.details = allDetails;
                saveBatchState(state);
                updateStatus('Batch cancelled — ' + totalProcessed + ' processed, ' + totalErrors + ' errors', 'warning');
                batchCancelled = false;
            ...
            if (batchStopped) {
                ...
                state.details = allDetails;
                saveBatchState(state);
                updateStatus('Batch paused — ' + totalProcessed + ' processed. Click Resume to continue.', 'success');
                batchStopped = false;
                lastBatchLog = allDetails;
                finishBatch('paused');
                return;
            ...
            var currentFileName = chunk[0].replace(/\\/g, '/').split('/').pop();
            var selTypes = options.selectedTypes || [];
            updateProgressOverlay(current + 1, chunks.length, currentFileName, totalProcessed, totalErrors, filesToProcess.length, selTypes);
            setProgressStatus('Processing chunk ' + (current + 1) + ' of ' + chunks.length + '...');
            updateStatus('[' + (current + 1) + '/' + chunks.length + '] ' + currentFileName + '...', 'processing');
            ...
            csInterface.evalScript(script, function(result) {
                    ...
                    if (data.details) {
                        data.details.forEach(function(d) {
                            ...
                            if (d.errors && d.errors.length > 0) {
                                failedFiles.push({file: d.file, errors: d.errors});
                            } else {
                                completedFiles.push(d.file);
                            }
                        });
                    }
                } catch (e) {
                ...
                state.currentChunkIndex = current + 1;
                state.completedFiles = completedFiles;
                state.failedFiles = failedFiles;
                state.details = allDetails;
                saveBatchState(state);
                ...
                current++;
                if (current < chunks.length) {
                    updateProgressOverlay(current, chunks.length, 'Resting...', totalProcessed, totalErrors, filesToProcess.length, selTypes);
                    setProgressStatus('Resting 2s before next chunk...');
    ...
    function startBatchFromPreview() {
        if (!pendingBatch) return;
        processBatchFolderByFolder(pendingBatch.options, pendingBatch.files);
        pendingBatch = null;
    }

    function resumeBatch(resumeState) {
        hideResumeBanner();
        if (!resumeState) {
            resumeState = loadBatchState();
        }
        if (!resumeState || resumeState.status !== 'paused') {
            updateStatus('No paused batch to resume', 'error');
            return;
        }
        updateStatus('Resuming batch from chunk ' + (resumeState.currentChunkIndex + 1) + '...', 'processing');
        processBatchFolderByFolder(resumeState.options, resumeState.totalFiles, resumeState);
    }

    function stopBatch() {
        batchStopped = true;
        updateStatus('Stopping batch at end of current chunk...', 'processing');
        setProgressStatus('Stopping at end of current chunk...');
    }
    ...
    function processDocument() {
        ...
        var runFixLayerOrder = document.getElementById('fixLayerOrderAction') ? document.getElementById('fixLayerOrderAction').checked : false;

        if (!runAyahAlign && !runOrnamentReplacement && !runFixLayerOrder) {
        ...
        var isAyahAlignOnly = (runAyahAlign && !runOrnamentReplacement && !runFixLayerOrder);

        if (!currentTemplate && !isAyahAlignOnly && runOrnamentReplacement) {
        ...
        var options = {
            ...
            deleteOldSwatches: elements.deleteOldSwatches ? elements.deleteOldSwatches.checked : false,
            autoSave: true,
            replacementMode: mode,
            runAyahAlign: runAyahAlign,
            runFixLayerOrder: runFixLayerOrder
        };
        ...
        if (options.mode === 'batch') {
            ...
            // Build file list and show batch preview
            var batchFiles = scanBatchFiles(options);
            if (!batchFiles || batchFiles.length === 0) {
                updateStatus('No files to process after filtering', 'error');
                return;
            }
            pendingBatch = { options: options, files: batchFiles };
            showBatchPreviewModal(batchFiles, options);
            return;
        }

        // Single file mode
        if (runFixLayerOrder && !runAyahAlign && !runOrnamentReplacement) {
            // Only Fix Layer Order selected - run directly
            fixCurrentLayerOrder();
            return;
        }
        ...
        csInterface.evalScript(script, function(result) {
            try {
                ...
                if (data.success) {
                    ...
                    if (runFixLayerOrder) {
                        successMsg += ' — now fixing layer order...';
                        fixCurrentLayerOrder();
                    } else {
    ...
    function fixLayerOrderForRiwayahBatch(riwayahName) {
        if (!riwayahName) {
            updateStatus('Please select a riwayah first', 'error');
            return;
        }
        var riwayahPath = mushafProjectFolder + '/mushaffiles/' + riwayahName;
        var dontSave = elements.silentMode ? elements.silentMode.checked : true;

        updateStatus('Fixing layer order for riwayah: ' + riwayahName + '...', 'processing');
        ensureLayerCopyScriptLoaded(function(ok) {
            if (!ok) {
                updateStatus('Failed to load layer copy script', 'error');
                return;
            }
            csInterface.evalScript('fixLayerOrderForRiwayah("' + riwayahPath.replace(/\\/g, '\\\\') + '", ' + (dontSave ? 'true' : 'false') + ')', function(result) {
                try {
                    var data = JSON.parse(result);
                    if (data.success) {
                        updateStatus('Layer order fixed for ' + riwayahName + ': ' + (data.message || 'Done'), 'success');
                    } else {
    ...
    function loadCheckboxSettings() {
        try {
            ...
            if (stored) {
                ...
                if (elements.deleteOldSwatches && settings.deleteOldSwatches !== undefined) {
                    elements.deleteOldSwatches.checked = settings.deleteOldSwatches;
                }
                ...
                if (fixLayerOrderAction && settings.fixLayerOrderAction !== undefined) {
                    fixLayerOrderAction.checked = settings.fixLayerOrderAction;
                }
                toggleFixLayerOrderSection();
                updateExecutionOrderBadge();
            }
    ...
    function saveActionSettings() {
        try {
            ...
            if (fixLayerOrderAction) settings.fixLayerOrderAction = fixLayerOrderAction.checked;
            localStorage.setItem(SETTINGS_KEY, JSON.stringify(settings));
        } catch(e) {}
    }
    ...
    if (elements.deleteOldSwatches) elements.deleteOldSwatches.addEventListener('change', saveCheckboxSettings);
    ...
    function loadRiwayahsFromMushafFiles() {
        ...
        if (elements.mushafProjectPath) {
            var pathParts = mushafProjectFolder.split('/').filter(function(p) { return p; });
            var displayPath = mushafProjectFolder;
            if (pathParts.length > 3) {
                displayPath = '.../' + pathParts.slice(-3).join('/');
            }
            elements.mushafProjectPath.textContent = displayPath;
        }
    ...
    function fixRiwayahLayerOrder() {
        ...
        var saveModeRadio = document.querySelector('input[name="saveMode"]:checked');
        var dontSave = saveModeRadio && saveModeRadio.value === 'dontSave';
    ...
    function populateFixOrderRiwayahDropdown() {
        if (elements.fixOrderRiwayahSelect) {
        ...
        if (elements.fixOrderRiwayahSelectLayer) {
            elements.fixOrderRiwayahSelectLayer.innerHTML = '<option value="">Select riwayah...</option>';
            availableRiwayahs.forEach(function(name) {
                var opt = document.createElement('option');
                opt.value = name;
                opt.textContent = name;
                elements.fixOrderRiwayahSelectLayer.appendChild(opt);
            });
        }
    }

    function fixRiwayahLayerOrderLayer() {
        var select = elements.fixOrderRiwayahSelectLayer;
        if (!select || !select.value) {
            updateStatus('Please select a riwayah first', 'error');
            return;
        }
        var riwayahPath = mushafProjectFolder + '/mushaffiles/' + select.value;
        var dontSave = elements.silentMode ? elements.silentMode.checked : true;

        if (elements.fixRiwayahLayerOrderBtnLayer) elements.fixRiwayahLayerOrderBtnLayer.disabled = true;

        updateStatus('Scanning riwayah files for layer order fix...', 'processing');
        ensureLayerCopyScriptLoaded(function(ok) {
            if (!ok) {
                if (elements.fixRiwayahLayerOrderBtnLayer) elements.fixRiwayahLayerOrderBtnLayer.disabled = false;
                return;
            }
            csInterface.evalScript('fixLayerOrderForRiwayah("' + riwayahPath.replace(/\\/g, '\\\\') + '", ' + (dontSave ? 'true' : 'false') + ')', function(result) {
                try {
                    var data = JSON.parse(result);
                    if (data.success) {
                        updateStatus('Layer order fixed for ' + select.value + ': ' + (data.message || 'Done'), 'success');
                    } else {
                        updateStatus(data.message || 'Fix failed', 'error');
                    }
                } catch (e) {
    ...
    function renderSurahRange() {
        ...
        for (var s = 1; s <= 114; s++) {
            var surahName = getSurahName(s);
            var surahLabel = surahName || ('surah' + String(s).padStart(3, '0'));
            var optFrom = document.createElement('option');
